...
void human_input()
{
    ...
    do
    {
        ...
        {
            cout<<"Input Error!Already piece here!"<<endl;
            check_number = -1;
        }
        else if(location<=0)
        {
            cout<<"Input Error!Foul number!"<<endl;
            check_number = -1;
        }
        else
        {
            cout<<"Input Error!Out of board range!"<<endl;
            check_number = -1;
        }
...
//实现评估函数
Evaluate evaluation(int chess_board[15][15])
{
    ...
    int virtual_board[17][17]; //建立虚拟棋盘记录边界
    for(int i=0;i<17;++i)
    {
        virtual_board[i][0]=3;
    }
    ...
    for(int i=1;i<=15;++i){
        for(int j=0;j<12;++j){
            type=chess_type[virtual_board[i][j]][virtual_board[i][j+1]][virtual_board[i][j+2]][virtual_board[i][j+3]][virtual_board[i][j+4]][virtual_board[i][j+5]];
            count_type[0][type]++;
        }
    }
    ...
    for(int j=1;j<=15;++j){
        for(int i=0;i<12;++i){
            type=chess_type[virtual_board[i][j]][virtual_board[i+1][j]][virtual_board[i+2][j]][virtual_board[i+3][j]][virtual_board[i+4][j]][virtual_board[i+5][j]];
            count_type[1][type]++;
        }
    }
    ...
    for(int i=0;i<12;++i){
        for(int j=0;j<12;++j){
            type=chess_type[virtual_board[i][j]][virtual_board[i+1][j+1]][virtual_board[i+2][j+2]][virtual_board[i+3][j+3]][virtual_board[i+4][j+4]][virtual_board[i+5][j+5]];
            count_type[2][type]++;
        }
    }
    ...
    Evaluate check;
    memset(check.keeper,0,sizeof(check.keeper));
    for(int i=1;i<17;i++)
    {
        ...
        if(i==WIN)
        {
            check.keeper[WIN]=counter;
        }
        else if(i==LOSE)
        {
            check.keeper[LOSE]=counter;
        }
    }
    //判断输赢
    check.result=0;
    if(check.keeper[WIN]>0)
    {
        check.result=-1;
    }
    else if(check.keeper[LOSE]>0)
    {
        check.result=1;
    }
    check.score=score_now;
    return check;
}

//简单局部搜索
bool no_neightnor(int i,int j) //每个点向八个方面延申两个长度，如果没有存在棋子则跳过
{
    for(int k=-2;k<=2;k++)
    {
        if(i+k>=0&&i+k<15&&chess_board[i+k][j]!=0)
            return true;
        else if(j+k>=0&&j+k<15&&chess_board[i][j+k]!=0)
            return true;
        else if(i+k>=0&&i+k<15&&j+k>=0&&j+k<15&&chess_board[i+k][j+k]!=0)
            return true;
        else if(i+k>=0&&i+k<15&&j-k>=0&&j-k<15&&chess_board[i+k][j-k]!=0)
            return true;
        else
...
//实现剪枝函数（先实现遍历棋盘再试着优化局部搜索）（最佳落子）
int cut(int &alpha,int dep,bool is_ai)//alpha记录上一层估计值
{
    bool a;
    if(dep==depth)
    {
        return evaluation(chess_board).score;
    }
    int beta,temp;//beta记录本层的极值，temp记录下层求得的估计值
    bool out=false;
    if(is_ai)
	{
		beta=10000;
    ...
    for(int i=0;i<15 && !out;i++)
    {
        for(int j=0;j<15 && !out;j++)
        {
            if(chess_board[i][j]==0)
            {
                ...
                {
                    chess_board[i][j]=1;
                    if(evaluation(chess_board).result==1)
					{
						temp=-10000;
					}
                    else
...
int main()
{
    initial_board();
    initial_chess_type();
    int m=-10000,alpha=-10000,dep=1; //m用来存储最大的alpha
	int x_pos,y_pos; //记录AI将要下的位置
	print();
	while(true)
    {
        human_input();
